...
${SegmentPrePrimary}
	...
	${AndIf} $2 != $3
		nsExec::Exec `"$EXEDIR\App\Bin\sqlite3.exe" "$EXEDIR\Data\MixxxSettings\mixxxdb.sqlite" "UPDATE directories SET directory = '$3' || SUBSTR(directory,(LENGTH('$2')+1)) WHERE fileName LIKE '$2%';"`
	${EndIf}
	...
	${If} ${FileExists} "$EXEDIR\Data\MixxxSettings\mixxxdb.sqlite"
	${AndIf} $2 != $3
		nsExec::Exec `"$EXEDIR\App\Bin\sqlite3.exe" "$EXEDIR\Data\MixxxSettings\mixxxdb.sqlite" "UPDATE LibraryHashes SET directory_path = '$3' || SUBSTR(directory_path,(LENGTH('$2')+1)) WHERE fileName LIKE '$2%';"`
		nsExec::Exec `"$EXEDIR\App\Bin\sqlite3.exe" "$EXEDIR\Data\MixxxSettings\mixxxdb.sqlite" "UPDATE track_locations SET directory = '$3' || SUBSTR(directory,(LENGTH('$2')+1)) WHERE fileName LIKE '$2%';"`
		nsExec::Exec `"$EXEDIR\App\Bin\sqlite3.exe" "$EXEDIR\Data\MixxxSettings\mixxxdb.sqlite" "UPDATE track_locations SET location = '$3' || SUBSTR(location,(LENGTH('$2')+1)) WHERE fileName LIKE '$2%';"`
	${EndIf}
